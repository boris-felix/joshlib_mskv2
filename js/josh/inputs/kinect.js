(function(J, $) {
    ...
    J.Inputs.kinect = J.Class(J.Input, {
        ...
        start: function() {
            ...
            this.cursorWait = this.getCursorWait(this.cursorWaitCanvas.getContext("2d"), 40, {
                x: this.cursorWidth / 2,
                y: this.cursorWidth / 2
            },
            10, {
                width: 2,
                height: 10
            ...
            this.cursorMove = this.getCursorMove(this.cursorCanvas.getContext("2d"), 50, {
                x: this.cursorWidth / 2,
                y: this.cursorWidth / 2
            },
            10, {
            ...
            var moveTo = function(x, y) {
                ...
                $(self.cursorCanvas).stop().animate({
                    "left": x - self.cursorWidth / 2,
                    "top": y - self.cursorWidth / 2
                },
                50, "linear");
        ...
        onStationary: function() {
            ...
            $(self.cursorWaitCanvas).css({
                "left": ((underElement[1] + underElement[3]) / 2 - self.cursorWidth / 2) + "px",
                "top": ((underElement[2] + underElement[4]) / 2 - self.cursorWidth / 2) + "px"
            });
    ...
    DepthJS.backend.connect = function() {
        ...
        return _.all(_.map(["event", "image", "depth"],
        function(stream) {
    ...
    DepthJS.backend.disconnect = function() {
        ...
        return _.map(["event", "image", "depth"],
        function(stream) {
            var oldSocket = DepthJS.backend[stream + "Ws"];
            if (oldSocket != null) {
                oldSocket.onmessage = null;
...
*/
 ...
 (function($) {
    ...
    $.idleTimer = function(newTimeout, elem) {
        ...
        var obj = $.data(elem, 'idleTimerObj') || new
        function() {};
